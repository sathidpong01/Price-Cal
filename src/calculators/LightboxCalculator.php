...
class LightboxCalculator extends BaseCalculator {
    private ?Material $lightboxType = null;
    private array $selectedOptions = [];
    private ?string $travelType = null;
    private ?float $distanceKm = null;

    /**
     * Set the lightbox type
     */
    public function setLightboxType(?Material $type): void {
    ...
    /**
     * Calculate the total price for the lightbox
     */
    public function calculate(): void {
        ...
        if ($this->lightboxType === null) {
            throw new \RuntimeException('Lightbox type must be selected');
        }
        ...
        $basePrice = $this->lightboxType->calculatePrice($areaSqm);
        $this->addPriceComponent(
            'ราคาพื้นฐาน',
            $basePrice,
            "{$areaSqm} ตร.ม. × {$this->lightboxType->price_per_unit} บาท/ตร.ม."
        );
